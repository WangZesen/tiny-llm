...
Submit jobs from the repository root using the [Slurm launcher](slurm.md#slurm-jobs).

## GH200 execution and profiling

Use the [SLURM launcher](slurm.md#slurm-jobs) on Arrhenius.
The synchronous 20M default uses microbatch 128 and a 131,072-token batch from
the [latest recipe sweep](../results/optimizer-tuning.md); 50M/90M use microbatch 32
...
See [GH200 measurements and analysis](../archive/gh200_performance.md).
...
now in the default presets. See [campaign results](../archive/campaign_results.md) for
...
[recipe](../methods/recipe.md). Rerun the same command after interruption to
...
Use the same [SLURM launcher and setup](slurm.md#slurm-jobs) for packed training on the
...
See [GH200 validation and benchmark results](../archive/packed_benchmarks.md) for the
